...
class DQNTrain:
    ...
    def train(self, n_episodes=1000,render=False, epsilon=EPSILON):
        ...
        # init DQN agent
        dqn_agent = QNetwork(hidden_size=hidden_size, learning_rate=learning_rate, state_size=self.obs_space_size, action_size=self.action_space_size)
...
class evoAgent:
    def __init__(self,env,averaged_over):
        ...
        self.obs_space_size = self.env.observation_space.spaces['image'].shape[0]**2
        self.action_space_size = self.env.action_space.n
        self.qnetwork = QNetwork(hidden_size=hidden_size, learning_rate=learning_rate,state_size=self.obs_space_size, action_size=self.action_space_size)
        self.memory = Memory(max_size=memory_size)
...
class EvoDQNTrain:
    def __init__(self,env="MiniGrid-Empty-16x16-v0",score_averaged_over=50,log_file = 'logs/log',cross_rate=CROSS_RATE,mutation_rate=MUTATION_RATE,reward_bound=REWARD_BOUND):
        ...
        self.obs_space_size = env_temp.observation_space.spaces['image'].shape[0]**2
