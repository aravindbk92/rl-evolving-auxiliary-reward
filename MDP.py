...
class MDP:
    ...
    def valueIteration(self,initialV,nIterations=np.inf,tolerance=0.01):
        ...
        while iterId <= nIterations:
            V = np.max(self.R + self.discount * np.matmul(self.T,V_old), axis=0)
    ...
    def evaluatePolicy(self,policy):
        ...
        R_pi = self.R[policy.tolist(), range(self.nStates)]
        T_pi = self.T[policy.tolist(), range(self.nStates)]
    ...
    def evaluatePolicyPartially(self,policy,initialV,nIterations=np.inf,tolerance=0.01):
        ...
        iterId = 1
        while iterId <= nIterations:
            V = R_pi + self.discount * np.matmul(T_pi,V_old)
            epsilon = np.linalg.norm(V_old - V)
            if epsilon < tolerance:
                break
            else:
